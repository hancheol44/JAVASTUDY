...
/*
  	반복문 
  		 ...
  		 1. for 명령
  		 		...
  		 		실행순서]	
  		 			1 → 2 → 3 →  5 → 4 → 3 → 5 → 4 → 3 → .... → 조건식이 false 일때 까지 반복 후 → 6
  		 			
  		 		참고]
  		 			(2) 처음 딱 한번만 실행한다.
  		 			(2) 주로 카운터변수를 선언하고 초기화하는 목적으로 사용한다.
  		 			(2) 필요없으면 생략해도 무방하나 ; 은 남겨야한다.
  		 			(2) 에서 만들어진 카운터 변수는 for명령문 내에서만 사용하는 지역변수
  		 					(#지역변수 : 블럭({})을 빠져나가는 순간 사라지는 변수) ↑
  		 					
  		 					
  		 			(4) 부분은 반복문의 내용이 실행이 끝나면 읽는 부분이다.
  		 			==> 반복횟수만큼 반복한다.	
